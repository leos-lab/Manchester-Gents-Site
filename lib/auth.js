...
import { normalizeNextAuthUrl } from './canonicalAuthUrl';
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        return {
          ...
          photoNoticeAgreedAt: user.photoNoticeAgreedAt
            ? user.photoNoticeAgreedAt.toISOString()
            : null
        };
      }
    })
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      if (user) {
        ...
        token.photoNoticeAgreedAt = user.photoNoticeAgreedAt || null;
        token.sessionVersion = sessionVersion;
      } else {
        token.sessionVersion = token.sessionVersion || sessionVersion;
      }
      if (trigger === 'update' && session?.photoNoticeAgreedAt) {
        token.photoNoticeAgreedAt = session.photoNoticeAgreedAt;
      }
      return token;
    },
    async session({ session, token }) {
      ...
      if (token) {
        ...
        session.user.photoNoticeAgreedAt = token.photoNoticeAgreedAt || null;
